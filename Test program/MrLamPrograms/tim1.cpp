...
void wait(unsigned int i) {
  if (!i) return timer.poll();
  i += timer.tick;
  while (i != timer.tick) timer.poll();
}

void pwmLeft(unsigned int i) {    //input must be lest than 45000
  TIM1->CCR1 = i;
}

void pwmRight(unsigned int i) {   //input must be lest than 45000
  TIM1->CCR2 = i;
}
